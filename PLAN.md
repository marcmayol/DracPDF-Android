...
5. **PDF→Word y Word→PDF: DECIDIDO el 17-ago-2026 — entran las dos.** No existen pdf2docx ni mammoth en Android, así que se escriben a mano, que es lo mismo que el escritorio hace con ODT y RTF: un `.docx` es un ZIP con XML dentro, y leerlo y escribirlo con `java.util.zip` y el analizador XML de la plataforma no necesita ninguna dependencia nueva. Lo que se promete es el **contenido**, no la maquetación: texto, títulos deducidos con el mismo criterio que Markdown, párrafos y tablas. Ambas direcciones salen etiquetadas «reformateado»; prometer el diseño exacto sería mentir, y el escritorio ya aprendió que ninguna conversión de ofimática lo conserva
6. **Conversiones entrantes y escáner: DECIDIDO el 17-ago-2026 — entran las dos cosas, y son dos cosas distintas.**
   - **Convertir a PDF** lo que ya está en el teléfono: imágenes, Markdown, HTML y texto, elegidos por SAF. Es la simétrica de las salientes y vive en el mismo verbo
   - **Escanear con la cámara**: capturar hojas y montarlas en un PDF. No es una conversión aunque acabe en lo mismo: pide permiso de cámara, vista previa, recorte y corrección de perspectiva, y por eso tiene su propia entrada y su propia pantalla en vez de esconderse dentro de «Convertir»
...
**Criterio F9 bis** (lo decidido el 17-ago-2026): un `.docx` escrito por la aplicación se vuelve a leer y devuelve el mismo texto y los mismos títulos, y uno ajeno —hecho con Word o LibreOffice— se convierte a PDF sin perder texto; las entrantes producen un PDF con tantas páginas como ficheros o imágenes se dieron. El escáner es demostración en dispositivo con el usuario: la cámara no se automatiza.
